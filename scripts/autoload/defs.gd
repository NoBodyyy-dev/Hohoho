...
const ITEMS := {
	...
	"perfume": {
		...
		"desc": "Флакон «Сирень-ностальгия». Облако сбивает грабителя с толку: голова кружится, все примеченные ловушки — забыты.",
		"only_char": "",
	},
	"wire": {
		"name": "Провод", "cost": 1, "place_time": 0.0,
...
const ANY_SURFACE := ["mousetrap", "tape", "garland_shock", "firecracker"]

const TRAP_BUDGET := 10        # активных ловушек на игрока — против спама
const WIRE_MAX_LEN := 12.0     # максимум длины провода, метров
...
const SUITCASE_LOADOUT := {"shards": 2, "tape": 1, "rope": 2, "mousetrap": 2, "banana": 1, "marbles": 1, "plate": 1, "wire": 1}
