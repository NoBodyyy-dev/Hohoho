...
**Актуализировано 2026-07-13:** раздел «Мебель» переписан с тегами
разрушаемости (🗄️🪚💥🔨⚡🎯, легенда — в начале раздела) и разбит по
приоритету геймплея. Добавлены недостающие предметы: chair, door_interior,
window_frame. Итого в списке — 76 моделей (24 готово).
...
### Приоритет 4 — двери и проёмы (сейчас процедурные — заменить моделью)

⬜ **door_interior.glb** 🔨 (межкомнатная дверь — взламывается, петли снимаются)
`low-poly, clean cartoon style, soft matte colors, smooth simple shapes, subtle bevels, no fine detail, readable silhouette, neutral studio lighting, single object, centered, upright, style of a cozy co-op party game, a simple flat wooden interior door with a round handle and three visible hinges on one side, closed, not a door frame, not double doors`

⬜ **window_frame.glb** 🔨 (окно — заколачивается/взламывается)
`low-poly, clean cartoon style, soft matte colors, smooth simple shapes, subtle bevels, no fine detail, readable silhouette, neutral studio lighting, single object, centered, upright, style of a cozy co-op party game, a simple wooden window frame with two glass panes and a small latch, closed, not a mirror, not a picture frame`
